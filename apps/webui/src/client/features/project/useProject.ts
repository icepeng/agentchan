...
export function useProject() {
  ...
  const activateProject = async (slug: string): Promise<Awaited<ReturnType<typeof fetchSessions>>> => {
    ...
    const sessions = await mutate<Awaited<ReturnType<typeof fetchSessions>>>(qk.sessions(slug));
    return sessions ?? [];
  };
