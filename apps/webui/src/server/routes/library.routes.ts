...
export function createLibraryRoutes() {
  ...
  app.get("/skills", async (c) => {
    return c.json(await c.get("libraryService").listSkills());
  });
  ...
  app.post("/skills", async (c) => {
    ...
    try {
      await c.get("libraryService").createSkill(name.trim(), content);
  ...
  app.get("/skills/:name", async (c) => {
    ...
    const content = await c.get("libraryService").getSkill(name);
  ...
  app.put("/skills/:name", async (c) => {
    ...
    try {
      await c.get("libraryService").updateSkill(name, content);
  ...
  app.delete("/skills/:name", async (c) => {
    const name = c.req.param("name");
    try {
      await c.get("libraryService").deleteSkill(name);
      return c.json({ ok: true });
    } catch (err: unknown) {
  ...
  app.get("/renderers", async (c) => {
    return c.json(await c.get("libraryService").listRenderers());
  });
  ...
  app.post("/renderers", async (c) => {
    ...
    try {
      await c.get("libraryService").createRenderer(name.trim(), source);
      return c.json({ ok: true }, 201);
    } catch (err: unknown) {
  ...
  app.get("/renderers/:name", async (c) => {
    const name = c.req.param("name");
    const source = await c.get("libraryService").getRenderer(name);
  ...
  app.put("/renderers/:name", async (c) => {
    ...
    try {
      await c.get("libraryService").updateRenderer(name, source);
      return c.json({ ok: true });
    } catch (err: unknown) {
  ...
  app.delete("/renderers/:name", async (c) => {
    const name = c.req.param("name");
    try {
      await c.get("libraryService").deleteRenderer(name);
      return c.json({ ok: true });
    } catch (err: unknown) {
