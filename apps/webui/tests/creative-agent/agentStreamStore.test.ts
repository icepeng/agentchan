...
import { registerAbortController } from "@/client/creative-agent/session/index.js";
import { recordAgentEvent } from "@/client/creative-agent/useRecordAgentEvent.js";
import { createAgentEventBus } from "@/client/creative-agent/stream/agentEventBus.js";
import { registerAgentStreamStore } from "@/client/creative-agent/stream/cancelAgentRun.js";
import {
  createAgentStreamStore,
  toAgentRunStatus,
} from "@/client/creative-agent/stream/agentStreamStore.js";
...
describe("agent run status", () => {
  test("prioritizes error over streaming over idle", () => {
    expect(
      toAgentRunStatus(state({ errorMessage: "boom", isStreaming: true })),
    ).toEqual({ kind: "error", message: "boom" });
    expect(toAgentRunStatus(state({ isStreaming: true }))).toEqual({
      kind: "streaming",
    });
    expect(toAgentRunStatus(IDLE_AGENT_STATE)).toEqual({ kind: "idle" });
  });
});
...
describe("agent stream store", () => {
  ...
  test("cancelAgentRun aborts and dispatches CLOSE idempotently", async () => {
    const store = createAgentStreamStore();
