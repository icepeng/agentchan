...
**Phase 6과의 차이**: simplify는 범용 코드 품질(재사용/중복/효율) — backend 포함한다. 이 Phase는 React/Next.js 특화 best practice(훅 규칙, 렌더 최적화, composition 패턴, Suspense/streaming 등) 검토다. 두 관심사는 교집합이 적으므로 Phase 6 결과를 근거로 이 Phase를 생략하지 않는다.

**Skip 조건**: `src/client/` 하위에 `.ts` 또는 `.tsx` 변경이 하나도 없을 때에만 건너뛴다. 그 외 어떤 이유(시간 절약, 다른 Phase와의 중복 인상, "사소한 변경")로도 skip하지 않는다. Skip할 때는 skip 사유를 사용자에게 명시적으로 보고한다.
...
서브에이전트의 보고를 **각 항목별로** 검토한다:
- 수정 필요 → 직접 수정
- 의도적 설계 결정이어서 무시 → 무시 사유를 한 줄 기록

보고 전체를 무시하거나 서브에이전트 호출 자체를 생략하지 않는다. 수정이 전혀 필요 없다고 판단한 경우에도, 그 판단의 근거를 사용자에게 간단히 보고한다.
...
- **Phase는 각자의 skip 조건으로만 건너뛴다.** 다른 Phase가 비슷한 일을 했다는 인상이나 시간 절약은 skip 사유가 아니다. Skip할 때는 해당 Phase에 명시된 skip 조건을 만족했음을 사용자에게 확인 보고한다.
