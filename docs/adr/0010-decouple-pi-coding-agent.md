...
Cherry-pick 정책은 Agentchan production write path가 없는 Pi custom message variant를 그대로 유지하지 않는다. `appendAtLeaf` 호출 지점과 entry construction 지점을 grep해 실제 영속화되는 entry가 없으면 vendored augmentation과 LLM conversion surface에서 제거할 수 있다.
...
- `buildSessionContext`는 Agentchan의 live variant subset에 대해 Pi와의 parity test로 drift를 감지한다.
