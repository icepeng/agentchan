...
에이전트가 플레이어 역할로 서버에 요청을 보내고, 템플릿이 **주장하는 규칙**이 실제 턴에서 지켜지는지 관측한다.
...
- Play state: !`cat .agents/skills/playtest/scripts/.play-state.json 2>/dev/null || echo "(none)"`

## CLI

스크립트: `bun .agents/skills/playtest/scripts/play.ts <cmd>`
...
- `.play-state.json`은 스키마 변경 시 폐기 (`conversationId` → `sessionId`) — 오래된 state가 남아있으면 삭제 후 `new`로 재시작
