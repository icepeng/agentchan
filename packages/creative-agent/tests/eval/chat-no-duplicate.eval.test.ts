 ...
 *
 * The duplicate response pattern occurs when the LLM:
 * 1. Calls write/append with RP content (narration, dialogue)
 * 2. Also outputs the same content as a text block in the same turn
 *
 * This causes the user to see the content twice — once in the rendered output
 * and once in the agent chat panel.
 */
...
suite("character-chat: no duplicate response", () => {
  ...
  test(
    ...
    async () => {
      harness = await EvalHarness.create({ skillNames: CHAT_SKILLS });
