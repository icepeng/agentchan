...
//   iframe 안의 독립 앱 — files fetch + SSE 상태 구독 + setTheme RPC.
// ─────────────────────────────────────────────────────────────────────────────
...
// pi-ai content blocks (inline — @agentchan/types 가 다루지 않는 세부 block).
...
// 렌더러가 쓰는 RenderContext는 module-level state/files 를 감싸는 얇은 synthetic.
...
// ── Bootstrap: module-level entry ─────────────────────────────

// STYLES 블록은 head에 1회 주입 (index.html 내부 <style> 는 base 레이아웃용).
const styleEl = document.createElement("div");
styleEl.innerHTML = STYLES;
const styleTag = styleEl.firstElementChild;
...
// ── Dynamic theme sync ────────────────────────
// mode 변경 시 setTheme RPC로 host에 팔레트 전환 요청.

let lastMode: "peace" | "combat" | null = null;

async function syncTheme(): Promise<void> {
  const mode = detectCurrentMode(currentCtx());
  if (mode === lastMode) return;
  lastMode = mode;
  const theme = mode === "combat" ? COMBAT_THEME : PEACE_THEME;
  try {
    ...
    console.warn("[three-winds-ledger] setTheme failed", err);
  }
}
