...
### 엔딩 분기
수사는 무한히 이어지지 않습니다. 유저의 공식 지목·유도 자백·OOC 종료 선언·씬 수 누적(8개 이상)이 엔딩 트리거. 네 가지 결말이 있습니다:

- **완전 해결** — 범인 정답 + 단서 3개 이상 해금 + 고신뢰 자백 또는 결정타. 표면 동기 뒤의 진짜 그림(반전)이 이때 공개됩니다
- **부분 해결** — 범인 정답이지만 단서 부족. 자백은 받지만 동기는 어둠 속
- **오판** — 엉뚱한 NPC를 단서 부족 상태에서 지목. 에필로그에 희미한 진실 암시만
- **열린 결말** — 지목 없이 세션 종료. 다음 세션의 여지

엔딩 씬은 `---` 구분선으로 분리되고, `---` 하나가 더 붙은 뒤 짧은 에필로그가 이어집니다. 엔딩 후 `stats.md`는 **리셋되지 않습니다** — 세션의 감정 상태가 그대로 남게.
