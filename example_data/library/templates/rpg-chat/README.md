...
- **페르소나 부트스트랩** — `files/personas/` 가 비어있는 첫 세션에서 `prepare-persona` 스킬이 한 번 호출되어 검사·도적·학자 프리셋 또는 자유 설정으로 페르소나 1개를 만듭니다. 이후 세션은 스킬 미호출 (context 절약).
- **캐릭터/페르소나/세계관/장면** — 캐릭터 챗과 같은 `files/` 구조입니다.

## 시작하는 한 줄

- **초보 모험가 1레벨. 마을 광장에서 시작.**
- **장비 없이 해적선 갑판에서 깨어나는 장면. HP 12/20으로 시작.**
