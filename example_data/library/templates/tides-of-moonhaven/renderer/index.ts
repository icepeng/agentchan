...
//   iframe 안의 독립 앱 — files fetch + SSE + setTheme RPC.
// ─────────────────────────────────────────────────────────────────────────────
...
let state: AgentState = EMPTY_AGENT_STATE;
let files: ProjectFile[] = [];

// Module-level character builder state — "순수 함수" 가정이 사라지면서
// form 값을 DOM 대신 JS heap에 직접 유지한다. renderEmpty() 재호출에도 값 보존.
const builderStats: Record<string, number> = {};
...
function currentCtx(): RenderContext {
  return { files, baseUrl, state };
}

// ── Renderer theme contract (inline) ─────────
...
// ── Bootstrap: inject STYLES once ─────────────

const styleHost = document.createElement("div");
styleHost.innerHTML = STYLES;
const styleTag = styleHost.firstElementChild;
if (styleTag) document.head.appendChild(styleTag);

function buildHTML(): string {
  return render(currentCtx());
}

function renderToRoot(): void {
  Idiomorph.morph(root, buildHTML(), { morphStyle: "innerHTML" });
  // morph 후 builder DOM 이 존재하면 현재 module state 를 동기화.
  syncBuilderDomFromState();
}

// ── Dynamic theme sync ────────────────────────
// mode 변경 시 setTheme RPC로 host 전역 팔레트 전환.

let lastMode: "peace" | "combat" | null = null;

async function syncTheme(): Promise<void> {
  const mode = readWorldMode(currentCtx());
  if (mode === lastMode) return;
  lastMode = mode;
  const nextTheme = mode === "combat" ? COMBAT_THEME : PEACE_THEME;
  try {
    await fetch(`${baseUrl}/actions/setTheme`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ theme: nextTheme }),
    });
  } catch (err) {
    console.warn("[tides-of-moonhaven] setTheme failed", err);
  }
}
...
// ── Character builder (module-level state) ─────────
// 원본 renderEmpty()의 inline <script>를 module-level 이벤트 위임으로 대체.
// 기존 BUILDER_TOTAL/BUILDER_MIN/BUILDER_MAX 상수는 spliced render 코드가 사용 중이므로
// 여기선 그 값만 재사용한다 (STAT_KEYS 포함).

function ensureBuilderStats(): void {
  for (const k of STAT_KEYS) {
    if (typeof builderStats[k] !== "number") builderStats[k] = 0;
  }
}

function sumBuilderStats(): number {
  let total = 0;
  for (const k of STAT_KEYS) total += builderStats[k] ?? 0;
  return total;
}

function findBuilderForm(): HTMLElement | null {
  return document.querySelector<HTMLElement>('.lg-builder[data-builder="1"]');
}

// morph 후 호출 — 현재 builder state 를 새 DOM 에 반영.
function syncBuilderDomFromState(): void {
  const form = findBuilderForm();
  if (!form) return;
  ensureBuilderStats();

  const nameInput = form.querySelector<HTMLInputElement>("[data-builder-name]");
  if (nameInput && nameInput.value !== builderName) {
    nameInput.value = builderName;
  }

  for (const k of STAT_KEYS) {
    const el = form.querySelector<HTMLElement>(`[data-stat-value="${k}"]`);
    if (!el) continue;
    const value = builderStats[k] ?? 0;
...
function refreshBuilderUI(): void {
  const form = findBuilderForm();
  if (!form) return;
  const total = sumBuilderStats();
  const totalEl = form.querySelector<HTMLElement>("[data-total-value]");
  const totalBox = form.querySelector<HTMLElement>("[data-builder-total]");
  if (totalEl) totalEl.textContent = String(total);
  if (totalBox) {
    totalBox.dataset.state =
      total === BUILDER_TOTAL
        ? "ok"
        : total > BUILDER_TOTAL
          ? "over"
  ...
  for (const k of STAT_KEYS) {
    const stepper = form.querySelector<HTMLElement>(
      `.lg-customs-stepper[data-stat="${k}"]`,
    );
    if (!stepper) continue;
    const value = builderStats[k] ?? 0;
    const minusBtn = stepper.querySelector<HTMLButtonElement>('[data-inc="-1"]');
    const plusBtn = stepper.querySelector<HTMLButtonElement>('[data-inc="1"]');
    if (minusBtn) minusBtn.disabled = value <= BUILDER_MIN;
    if (plusBtn)
      plusBtn.disabled = value >= BUILDER_MAX || total >= BUILDER_TOTAL;
  }

  const submit = form.querySelector<HTMLButtonElement>("[data-builder-submit]");
  const valid = total === BUILDER_TOTAL && builderName.trim().length > 0;
  if (submit) {
    submit.disabled = !valid;
    if (valid) {
      const statLine = STAT_KEYS.map((k) => `${k} ${builderStats[k] ?? 0}`).join(" ");
      submit.dataset.text = `/init\n이름: ${builderName.trim()}\n스탯: ${statLine}`;
    } else {
      submit.dataset.text = "";
    }
  }

  const errorEl = form.querySelector<HTMLElement>("[data-builder-error]");
  if (errorEl) {
    errorEl.hidden = true;
    errorEl.textContent = "";
  }
...
document.addEventListener("click", (e) => {
  ...
  // Stat stepper (builder)
  const stepBtn = el.closest<HTMLElement>("[data-inc]");
  if (stepBtn) {
    const form = stepBtn.closest<HTMLElement>('.lg-builder[data-builder="1"]');
    if (form) {
      e.preventDefault();
      e.stopPropagation();
      const stepper = stepBtn.closest<HTMLElement>(".lg-customs-stepper");
      const key = stepper?.dataset.stat as (typeof STAT_KEYS)[number] | undefined;
      if (!key || !STAT_KEYS.includes(key)) return;
      ensureBuilderStats();
      const delta = parseInt(stepBtn.dataset.inc ?? "0", 10) || 0;
      const current = builderStats[key] ?? 0;
      const next = current + delta;
      if (next < BUILDER_MIN || next > BUILDER_MAX) return;
      if (delta > 0 && sumBuilderStats() >= BUILDER_TOTAL) return;
      builderStats[key] = next;
      const valueEl = form.querySelector<HTMLElement>(`[data-stat-value="${key}"]`);
      if (valueEl) {
  ...
  // Submit validation guard (builder 전용)
  const submitBtn = el.closest<HTMLElement>("[data-builder-submit]");
  if (submitBtn) {
    const form = submitBtn.closest<HTMLElement>('.lg-builder[data-builder="1"]');
    if (form) {
      const total = sumBuilderStats();
      const nameOk = builderName.trim().length > 0;
      if (total !== BUILDER_TOTAL || !nameOk) {
      ...
      // 검증 통과 시 아래 data-action="send" 핸들러가 이어서 처리한다.
    }
  }
  ...
  if (action === "send" || action === "fill") {
    void fetch(`${baseUrl}/actions/${action}`, {
      method: "POST",
      headers: { "content-type": "application/json" },
...
document.addEventListener("input", (e) => {
  ...
  if (!el) return;
  if (el.matches("[data-builder-name]")) {
    builderName = el.value;
    refreshBuilderUI();
  }
});
...
document.addEventListener(
  ...
  (e) => {
    const el = e.target as HTMLElement | null;
    if (!el || !el.matches("[data-builder-name]")) return;
    const sig = el.closest<HTMLElement>("[data-sig]");
    if (sig) sig.dataset.focused = "1";
  },
  true,
);
...
document.addEventListener("keydown", (e) => {
  ...
  if (e.key === "Enter" && !(e as KeyboardEvent).isComposing) {
    e.preventDefault();
    const submit = document.querySelector<HTMLButtonElement>("[data-builder-submit]");
...
const sse = new EventSource(`${baseUrl}/state/stream`);
...
sse.addEventListener("streaming_clear", () => {
  ...
  void loadFiles().then(renderToRoot);
});
