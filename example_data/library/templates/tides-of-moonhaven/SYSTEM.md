...
[SYSTEM] 판정: 힘 DC 18 → 성공 (주사위 18)        ← 보정 0일 때는 주사위만
...
```
[CHOICES]
- label: 지하실 문을 부순다 | action: 지하실 문을 강제로 연다 | stat: 힘 | dc: 18
...
- `dc` (선택) — 난이도 (6 단순 / 8 쉬움 / 10 일상 / 12 보통 / 15 어려움 / 18 매우 어려움 / 22 거의 불가능)
- 필드는 ` | ` 로 구분

#### 선택지 DC 배분

- `[CHOICES]` 2-4개 중 **무판정 선택지**를 최소 1개 둔다.
- 한 블록에서 `stat`/`dc`가 붙은 선택지는 보통 0-1개, 긴박한 장면에서도 최대 2개.
- DC를 기본값처럼 붙이지 않는다. 실패가 장면을 바꿀 때만 판정 선택지로 만든다.
- DC 10-15에 몰리지 않게 상황 규모를 먼저 정한다:
  - DC 6-8: 위험은 낮지만 우연·시간·소음이 변수인 행동
  - DC 10-12: 평범한 장애물, 준비 없이도 시도 가능한 행동
  - DC 15: 실패 시 단서 지연·관계 악화·피해 같은 대가가 있는 행동
  - DC 18: 경계 중인 상대, 악천후, 복잡한 장치처럼 명백히 불리한 행동
  - DC 22: 거의 불가능한 도박. 중요한 장면에서만 드물게
- 대화, 이동, 질문, 주변 관찰처럼 이야기를 여는 행동은 대개 `dc` 없이 둔다.
...
```
(stats.yaml: 통찰 = 2)
script(file: "skills/dice-roll/scripts/roll.ts", args: ["1d20+2", "8"])
→ 결과:
   Notation: 1d20+2
   Roll: 14
   Modifier: +2
...
[SYSTEM] 판정: 통찰 DC 8 → 성공 (주사위 14 +통찰 2 = 16) — 벽 뒤에서 미세한 바람이 느껴진다
...
[CHOICES]
- label: 벽을 살펴본다 | action: 벽 뒤의 바람을 추적한다 | stat: 통찰 | dc: 8
...
- 판정 빈도는 낮게 유지. 사용자가 선택지를 누를 때마다 굴리지 말고, 위험·비용·정보 잠금이 실제로 있는 순간에만 굴린다
