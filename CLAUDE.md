...
- **Templates**: 프로젝트 생성용 프리셋 목록 (`data/library/templates/`). `README.md` frontmatter(name/description) + SYSTEM.md + skills/ + renderer.ts + files/
...
- **Template repo**: `createTemplateRepo(dir)` — 디렉토리 기반 CRUD. `README.md` frontmatter가 메타데이터의 단일 소스(name/description) + body는 사용 가이드. README가 없는 디렉토리는 `list()`에서 제외됨
...
│       └── {template-name}/
│           ├── README.md       # frontmatter: name/description (메타데이터 단일 소스) + body: 사용 가이드
│           ├── SYSTEM.md
│           ├── renderer.ts
│           ├── skills/
│           └── files/
...
- 구조: `example_data/library/templates/{name}/` (프로젝트 프리셋 — README.md + SYSTEM.md + skills/ + renderer.ts + files/)
