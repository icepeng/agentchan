...
- **Library**: 3탭 (Skills, Renderers, Systems). `LibraryEditorConfig<T>`로 탭별 CRUD 추상화
- **Project Settings**: 4탭 (General, System, Skills, Renderer). System/Renderer 탭은 `ProjectLibraryTab` 공통 컴포넌트 (config 기반). Library에서 프로젝트로 복사하는 "pick + preview + apply" UX
...
- **Library repo 패턴**: `createFlatFileCrud(dir, ext, label)` 헬퍼로 renderers/systems의 동일한 CRUD를 공유. Skills는 서브디렉토리 기반이라 별도 구현

## Client Layer Architecture (FSD-inspired)
```
app/              ← 최상위: UIContext, providers 조합, AppShell, Sidebar
pages/            ← PageRoute 1:1 매핑 (조합만, 로직 없음)
features/         ← 도메인 UI + 오케스트레이션 훅 (chat, project, library, settings, onboarding)
...
```
apps/webui/data/
├── library/                    # Shared library (skills + renderers + systems)
│   ├── skills/                 # Library skills (SKILL.md in subdirs)
│   │   └── {skill-name}/SKILL.md
│   ├── renderers/              # Library renderers (.ts files)
│   │   └── {name}.ts
│   └── systems/                # Library SYSTEM.md templates (.md files)
│       └── {name}.md
...
- Library skills → 프로젝트에 복사하여 사용. catalog은 system prompt에 위치
